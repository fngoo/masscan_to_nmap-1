...
def portscan():
    ...
    ports = []
    print './masscan/bin/masscan -iL scan_ip.txt -p 1-65535 -oJ masscan.json --rate 266'
    os.system('./masscan/bin/masscan -iL scan_ip.txt -p 1-65535 -oJ masscan.json --rate 266')
    # 提取json文件中的端口
...
def Title(scan_url_port, service_name):
    try:
        r = requests.get(scan_url_port, timeout=15, verify=False, stream=True)
        # 获取网站的页面编码
